...
document.addEventListener('DOMContentLoaded',()=>{
  start.addEventListener('click',async () => {
    ...
    console.log("check")
    // Tong = checkLocation(Tong) 
    Tong = await checkLocation(Tong) 
    time.style.display = "";
    ...
    stop.addEventListener('click',()=>{
        let inputUser = prompt('Bạn có muốn tham gia lại thử thách  không ? V(yes)/X(no)').toLocaleLowerCase()
        if(inputUser === 'v'){
        ...
        setTimeout(function(){window.location.reload('https://runforlifeweb-2020.web.app/')},2000) 
            ...
            clearInterval(reapeat)
            start.style.display = "none"
            if(randomNumber > Tong || SecondsTime > 60){
                document.getElementById('check_game').innerText = "GAME OVER"
            }
            else if(randomNumber <= Tong){
                document.getElementById('check_game').innerText = "WIN"
            }
        }
    })
    }
